...
class AudioStreamer:
    ...
    def start_stream(self, mic_a_id, mic_b_id, output_id, enable_output):
        ...
        try:
            ...
            self.process_thread = threading.Thread(target=self._processing_loop, args=(enable_output, output_id))
            self.process_thread.start()

            if self.shared_stereo:
                # I2S Hardware / Single Stereo Interface (e.g. Raspberry Pi 5)
                self.stream_a = sd.InputStream(
                    device=mic_a_id, channels=2, samplerate=self.sr_a,
                    blocksize=512, callback=self._stereo_callback
                )
                self.stream_a.start()
            else:
                # Two separate USB Microphones (e.g. Windows Testing)
                self.stream_a = sd.InputStream(
                    device=mic_a_id, channels=1, samplerate=self.sr_a,
                    blocksize=512, callback=self._mono_callback_a
                )
                self.stream_b = sd.InputStream(
                    device=mic_b_id, channels=1, samplerate=self.sr_b,
                    blocksize=512, callback=self._mono_callback_b
                )
                self.stream_a.start()
                self.stream_b.start()
    ...
    def _stereo_callback(self, indata, frames, time, status):
        if self.running:
            try:
                ...
                self.queue_b.put_nowait(indata[:, 1].copy() if indata.shape[1] > 1 else np.zeros_like(indata[:, 0]))
            except queue.Full:
                pass

    def _mono_callback_a(self, indata, frames, time, status):
        if self.running:
            try: self.queue_a.put_nowait(indata[:, 0].copy())
            except queue.Full: pass
            
    def _mono_callback_b(self, indata, frames, time, status):
        if self.running:
            try: self.queue_b.put_nowait(indata[:, 0].copy())
            except queue.Full: pass

    def _processing_loop(self, enable_output, output_id):
        if enable_output:
            self.stream_out = sd.OutputStream(device=output_id, channels=1, samplerate=self.sr_out, blocksize=0)
        ...
        resamp_out = soxr.Resample(48000, self.sr_out, 1) if self.sr_out != 48000 else None
        ...
        while self.running:
            try:
                ...
                if resamp_a: data_a = resamp_a.resample(data_a)
                if resamp_b: data_b = resamp_b.resample(data_b)
...
class ModernLiveUI(ctk.CTk):
    def __init__(self):
        ...
        self.logo_label = ctk.CTkLabel(self.sidebar_frame, text="SIH High-SPL SE", font=ctk.CTkFont(size=22, weight="bold"))
        self.logo_label.pack(pady=(30, 10), padx=20)
        
        self.info_label = ctk.CTkLabel(self.sidebar_frame, text="True Dual-Mic PLD + AI\nReal-Time Output Streaming", font=ctk.CTkFont(size=14), text_color="gray")
        ...
        # MIC A
        ctk.CTkLabel(self.main_frame, text="Mic A (Primary: Voice + Noise)", font=ctk.CTkFont(weight="bold", size=14)).pack(anchor="w", pady=(10, 0), padx=20)
        self.combo_a = ctk.CTkComboBox(self.main_frame, values=dev_list, width=450)
        self.combo_a.pack(anchor="w", pady=(2, 10), padx=20)

        # MIC B
        ctk.CTkLabel(self.main_frame, text="Mic B (Reference: Extreme Noise)", font=ctk.CTkFont(weight="bold", size=14)).pack(anchor="w", padx=20)
        self.combo_b = ctk.CTkComboBox(self.main_frame, values=dev_list, width=450)
        self.combo_b.pack(anchor="w", pady=(2, 10), padx=20)

        # OUTPUT
        ctk.CTkLabel(self.main_frame, text="Output Hardware (Bluetooth/Speaker)", font=ctk.CTkFont(weight="bold", size=14)).pack(anchor="w", padx=20)
        self.out_combo = ctk.CTkComboBox(self.main_frame, values=list(self.out_devices.keys()), width=450)
        self.out_combo.pack(anchor="w", pady=(2, 10), padx=20)

        # Output Toggle
        self.stream_output_var = ctk.BooleanVar(value=True)
        self.stream_switch = ctk.CTkSwitch(self.main_frame, text="Enable Real-Time Output Streaming", variable=self.stream_output_var, font=ctk.CTkFont(weight="bold"))
        ...
        self.start_btn = ctk.CTkButton(self.btn_frame, text="Start Streaming", command=self.toggle_stream, fg_color="#28a745", hover_color="#218838", font=ctk.CTkFont(weight="bold", size=15), height=40)
        self.start_btn.pack(side="left")

        self.status_label = ctk.CTkLabel(self.btn_frame, text="Status: IDLE", text_color="gray", font=ctk.CTkFont(size=14))
        self.status_label.pack(side="left", padx=20)

        # Meters
        ctk.CTkLabel(self.main_frame, text="Mic A Input Level", font=ctk.CTkFont(size=12)).pack(anchor="w", pady=(10, 0), padx=20)
        self.meter_in = ctk.CTkProgressBar(self.main_frame, width=450)
        self.meter_in.set(0)
        self.meter_in.pack(anchor="w", pady=(2, 5), padx=20)

        ctk.CTkLabel(self.main_frame, text="Processed Clean Output Level", font=ctk.CTkFont(size=12)).pack(anchor="w", padx=20)
        self.meter_out = ctk.CTkProgressBar(self.main_frame, width=450, progress_color="#28a745")
    ...
    def toggle_stream(self):
        if not self.streamer.running:
            ...
            if not a_name or not b_name or not out_name:
                self.status_label.configure(text="Error: Select all devices", text_color="#dc3545")
                return
            ...
            self.status_label.configure(text="Initializing Dual-Mic DSP...", text_color="#ffc107")
            self.update()

            success, msg = self.streamer.start_stream(id_a, id_b, id_out, do_output)
            if success:
                self.start_btn.configure(text="Stop Streaming", fg_color="#dc3545", hover_color="#c82333")
            ...
            self.start_btn.configure(text="Start Streaming", fg_color="#28a745", hover_color="#218838")
