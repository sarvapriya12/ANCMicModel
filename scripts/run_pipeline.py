...
def main():
    parser = argparse.ArgumentParser(
        description="End-to-End Pipeline for Audio Enhancement"
    )
    parser.add_argument(
        "--test-dir",
        type=str,
    ...
    parser.add_argument(
        "--backend", type=str, default="onnxruntime", choices=["pytorch", "onnxruntime"]
    )
    ...
    def process_audio_file(audio_path, ref_path=None):
        try:
            ...
            if ref_path:
                ...
                if sr != ref_sr:
                    print(
                        "  Warning: Sample rate mismatch between primary and reference!"
                    )
        ...
        while offset < audio.size:
            ...
            if ref_path:
                ...
                if len(chunk_a) == len(chunk_b) and len(chunk_a) == chunk_size:
                    chunk_a, nlms_state = nlms_engine.process(
                        chunk_a, chunk_b, nlms_state
                    )
