...
class HighSPLPipeline:
    def __init__(self):
        ...
        self.robust_nlms = RobustNLMS(
            pld_threshold_db=12.0, freeze_on_clipping=True, freeze_on_divergence=True
        )
        self.state_robust = self.robust_nlms.reset()

        # 2. Academic VSS-NLMS
        self.vss_nlms = VSSNLMS(
            filter_length=256, mu_max=0.5, mu_min=0.001, alpha=0.99, gamma=0.01
        )
        ...
        # 4. Battlefield FDAF
        self.battlefield_fdaf = BattlefieldFDAF(
            block_size=128, num_partitions=4, step_size=0.05
        )
    ...
    def process(self, primary: np.ndarray, reference: np.ndarray):
        ...
        if self.dsp_mode == "ROBUST":
            enhanced_audio_nlms, self.state_robust = self.robust_nlms.process(
                primary, reference, self.state_robust
            )
        elif self.dsp_mode == "VSS":
            enhanced_audio_nlms, self.state_vss = self.vss_nlms.process(
                primary, reference, self.state_vss
            )
        elif self.dsp_mode == "CLASSICAL":
            enhanced_audio_nlms, self.state_classical = self.classical_nlms.process(
                primary, reference, self.state_classical
            )
        elif self.dsp_mode == "FDAF":
            enhanced_audio_nlms, self.state_fdaf = self.battlefield_fdaf.process(
                primary, reference, self.state_fdaf
            )
        else:
