...
class SpectrumPlot(pg.PlotWidget):
    def __init__(self, title="GRAPH 2: SPECTRUM (Raw vs Enhanced)", sr=48000):
        ...
        self.curve_raw = self.plot(
            pen=pg.mkPen(
                color="#ff5500", width=1, style=pg.QtCore.Qt.PenStyle.DashLine
            ),
            name="Mic A Raw",
        )
        self.curve_enh = self.plot(
            pen=pg.mkPen(color="#00ff00", width=1.5), name="Enhanced"
        )
        self.addLegend()
